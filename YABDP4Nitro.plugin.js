          ...
          return class YABDP4Nitro extends Plugin {
                ...
                ...[
                  ...
                  new Settings.SettingGroup("Profile").append(
                    ...
                    new Settings.Switch(
                      ...
                      "Uses invisible 3y3 encoding to allow setting custom profile pictures by hiding an image URL in your bio. Only supports Imgur URLs for security reasons.",
                      this.settings.customPFPs,
                      (value) => (this.settings.customPFPs = value)
                    ),
                    new Settings.Switch(
                      "UserPFP Integration",
                      "Imports the UserPFP database so that people who have profile pictures in the UserPFP database will appear with their UserPFP profile picture.",
            ...
            clientThemes() {
              ...
              if (this.settings.lastGradientSettingStore != -1) {
                ...
                this.gradientSettingModule.updateBackgroundGradientPreset(
                  this.settings.lastGradientSettingStore
                ); //Restore gradient on plugin load/save
              }
            ...
            customProfilePictureDecoding() {
              if (this.getUserAvatarUrlModule == undefined)
                this.getUserAvatarUrlModule =
                  WebpackModules.getByPrototypes("getUserAvatarURL").prototype;
              BdApi.Patcher.instead(
                "YABDP4Nitro",
                this.getUserAvatarUrlModule,
                "getUserAvatarURL",
                (user, args, originalFunction) => {
            ...
            customProfilePictureEncoding(secondsightifyEncodeOnly) {
              function makePfpEncodingShit(self) {
                if (document.getElementById("profilePictureButton") != null)
                  return;
                if (self.containerClassModulePfp == undefined)
                  self.containerClassModulePfp = WebpackModules.getAllByProps(
                    "buttonsContainer",
                    "removeButton",
                    "buttonHighlighted"
                  )[1];
                const profileThemeSection = document.getElementsByClassName(
                  self.containerClassModulePfp.buttonsContainer
                );
                ...
                profilePictureButton.onclick = async function () {
                  ...
                  if (stringToEncode.toLowerCase().startsWith("imgur.com")) {
                    if (
                      stringToEncode.replace("imgur.com/", "").startsWith("a/")
                    ) {
                    ...
                    stringToEncode =
                      "P{" + stringToEncode.replace("imgur.com/", "") + "}";
            ...
            honorBadge() {
              ...
              function applyCustomBadgeIcon(self) {
                if (self.profileBadgesClass == undefined)
                  self.profileBadgesClass =
                    WebpackModules.getByProps("profileBadges").profileBadges;
                for (const element of document.getElementsByClassName(
                  self.profileBadgesClass
                ) +
                  document.getElementsByClassName(
                    DiscordClassModules.UserModal.profileBadge
                  )) {
                  const qry = document.querySelectorAll(
                    '[aria-label="A fellow YABDP4Nitro user!"]'
                  );
                  if (qry.length > 0) {
                    qry.forEach((obj) => {
                      const icon =
                        "https://raw.githubusercontent.com/riolubruh/riolubruh.github.io/main/badge.png";
                      const badge = obj;
                      if (badge.firstChild.src != icon)
                        badge.firstChild.src = icon;
                    });
                  }
                  const qry2 = document.querySelectorAll(
                    '[aria-label="YABDP4Nitro Creator!"]'
                  );
                  if (qry2.length > 0) {
                    qry2.forEach((obj) => {
                  ...
                  const qry3 = document.querySelectorAll(
                    '[aria-label="YABDP4Nitro Contributor!"]'
                  );
                  if (qry3.length > 0) {
                    qry3.forEach((obj) => {
                      const icon = "https://i.imgur.com/bYGGXnq.gif";
              ...
              if (this.profileRenderer == undefined)
                this.profileRenderer = WebpackModules.getAllByProps(
                  ...
                  obj.default.toString().includes("CLYDE_SETTINGS")
                )[0];
            ...
            profileFX(
              ...
              function makeProfileEffectButtons(self) {
                if (self.profileFxButtonContainerClass == undefined)
                  self.profileFxButtonContainerClass =
                ...
                if (
                  document.getElementById("changeProfileEffectButton") ==
                  undefined
                ) {
                  ...
                  for (let i = 0; i < profileEffects.length; i++) {
                    let previewURL =
                      profileEffects[i].config.thumbnailPreviewSrc;
                    profileEffectsHTML += `<img class="riolubruhsSecretStuff" src="${previewURL}" />${i}`;
                    if ((i + 1) % 4 == 0) {
                      //every 4th profile effect
                  ...
                  profileCustomizationSection.parentElement.append(
                    profileEffectsElement
                  );

                  let profileEffectButtons = document.getElementsByClassName(
                    "riolubruhsSecretStuff"
                  );
                  ...
                  for (let i = 0; i < profileEffectButtons.length; i++) {
                    ...
                    profileEffectButtons[i].outerHTML = profileEffectButtons[
                      i
                    ].outerHTML.replace(
                      "class",
                      `onclick='${copyDecoration3y3}' class`
                    );
                  }
                }
              }
            ...
            fakeAvatarDecorations(self) {
              if (this.shopModule == undefined)
                this.shopModule = WebpackModules.getAllByProps(
                  "default"
                ).filter((obj) =>
                  obj.default
                    .toString()
                    .includes("useFetchCollectiblesCategoriesAndPurchases")
                )[0];
                  ...
                  (_, args, ret) => {
                      ...
                      if (item.asset != undefined) {
                        if (self.settings.avatarDecorations != undefined) {
                          if (self.settings.avatarDecorations.length > 0) {
                            if (
                              self.settings.avatarDecorations.includes(
                                String(item.asset)
                              )
                            )
                              return;
                        ...
                        self.settings.avatarDecorations.unshift(
                          String(item.asset)
                        );
                    ...
                    function handleEachStoreItem(element) {
                      element.products.forEach((product) =>
                        handleEachProduct(product)
                      );
                    }
                    ret.categories.forEach((element) =>
                      handleEachStoreItem(element)
                    );
                  }
                );
              } catch (err) {
                console.log(
                  "[YABDP4Nitro] An error occurred while patching the shop module."
                );
                console.error(err);
              }
              ...
              BdApi.Patcher.after(
                ...
                (_, args, ret) => {
                  ...
                  if (
                    avatarDecorations == new Array() ||
                    avatarDecorations == undefined ||
                  ...
                  if (position == undefined) return;
                  let assetIndex = parseInt(
                    revealedText.slice(position + 2, position + 4)
                  );
              ...
              function makeAvatarDecorationShit(
                ...
                if (self.avatarDecorationButtonsContainerClass == undefined)
                  self.avatarDecorationButtonsContainerClass =
                    WebpackModules.getAllByProps(
                      "buttonsContainer",
                      "buttonHighlighted",
                      "removeButton"
                    )[3].buttonsContainer;
                let avatarDecorationSection = document.getElementsByClassName(
                  self.avatarDecorationButtonsContainerClass
                )[0];

                if (avatarDecorationSection != undefined) {
                  if (
                    document.getElementById("decorationButton") == undefined
                  ) {
                    ...
                    if (
                      document.getElementById("avatarDecorations") == undefined
                    ) {
                      ...
                      for (let i = 0; i < avatarDecorations.length; i++) {
                        //no more copy-pasted html, I can die happy
                        avatarDecorationsHTML +=
                          `<img class="riolubruhsspecialsauce" src="https://cdn.discordapp.com/avatar-decoration-presets/` +
                          avatarDecorations[i] +
                          `.png?size=64" /> ${i}`;
                        if ((i + 1) % 4 == 0) avatarDecorationsHTML += "<br>";
                      }
                      avatarDecorationsHTML += "</div>";
                      let avatarDecorationsElement =
                        document.createElement("div");
                      avatarDecorationsElement.innerHTML =
                        avatarDecorationsHTML;
                      avatarDecorationSection.parentElement.append(
                        avatarDecorationsElement
                      );

                      let avatarDecorationsElements =
                        document.getElementsByClassName(
                          "riolubruhsspecialsauce"
                        );
                      ...
                      for (
                        let i = 0;
                        i < avatarDecorationsElements.length;
                        i++
                      ) {
                        let encodedText = secondsightifyEncodeOnly("/a" + i);
                        let copyDecoration3y3 = `const clipboardTextElem = document.createElement("textarea"); clipboardTextElem.style.position = "fixed"; clipboardTextElem.value = " ${encodedText}"; document.body.appendChild(clipboardTextElem); clipboardTextElem.select(); clipboardTextElem.setSelectionRange(0, 99999); document.execCommand("copy"); ZLibrary.Toasts.info("3y3 copied to clipboard!"); document.body.removeChild(clipboardTextElem);`;
                        avatarDecorationsElements[i].outerHTML =
                          avatarDecorationsElements[i].outerHTML.replace(
                            "class",
                            `onclick='${copyDecoration3y3}' class`
                          );
                      }
              ...
              if (this.settings.CustomFPSEnabled) {
                if (this.CustomFPS != 60) {
                  ...
                  this.StreamButtons.ApplicationStreamFPSButtons[2].value =
                    this.settings.CustomFPS;
                  delete this.StreamButtons.ApplicationStreamFPSButtons[2]
                    .label;
            ...
            emojiBypass() {
              //Moved to a function to declutter saveAndUpdate
              //Upload Emotes
                  ...
                  (e) => {
                    ...
                    for (const ladder in e.videoQualityManager.ladder.ladder) {
                      e.videoQualityManager.ladder.ladder[ladder].framerate =
                        this.settings.CustomFPS;
                      e.videoQualityManager.ladder.ladder[
                        ladder
                      ].mutedFramerate = parseInt(this.settings.CustomFPS / 2);
                    }
                    for (const ladder of e.videoQualityManager.ladder
                      .orderedLadder) {
                      ladder.framerate = this.settings.CustomFPS;
                      ladder.mutedFramerate = parseInt(
                        this.settings.CustomFPS / 2
                      );
                    }
            ...
            encodeProfileColors(primary, accent) {
              if (this.themeColorsPickerModule == undefined)
                this.themeColorsPickerModule = WebpackModules.getByProps(
                  "getTryItOutThemeColors"
                );

              function makeCopyButtonAndEncoding(
                themeColorsPickerModule,
                self
              ) {
                const sectionContainerClassModule =
                  WebpackModules.getByProps("sectionContainer");
                const profileThemeSection = document.getElementsByClassName(
                  sectionContainerClassModule.sectionContainer
                );
                ...
                copyButton.onclick = function () {
                  ...
                  try {
                    themeColors =
                      themeColorsPickerModule.getAllTryItOut()
                        .tryItOutThemeColors;
                  } catch (err) {
                    console.warn(err);
                  }
                  if (themeColors == null) {
                    try {
                      themeColors =
                        themeColorsPickerModule.getAllPending()
                          .pendingThemeColors;
                    } catch (err) {
                      console.error(err);
                    }
            ...
            bannerUrlEncoding(secondsightifyEncodeOnly) {
              function makeBannerEncodingShit(self) {
                if (document.getElementById("profileBannerButton") != undefined)
                  return;
                if (self.containerClassModuleBanner == undefined)
                  self.containerClassModuleBanner =
                    WebpackModules.getAllByProps(
                      "buttonsContainer",
                      "removeButton",
                      "buttonHighlighted"
                ...
                const profileThemeSection = document.getElementsByClassName(
                  self.containerClassModuleBanner.buttonsContainer
                );
                const buttonClassModule = self.buttonClassModule;
                ...
                profileBannerButton.onclick = async function () {
                  ...
                  if (stringToEncode.toLowerCase().startsWith("imgur.com")) {
                    ...
                    stringToEncode =
                      "B{" + stringToEncode.replace("imgur.com/", "") + "}";
                    encodedStr = " " + secondsightifyEncodeOnly(stringToEncode);
                    Toasts.info("3y3 copied to clipboard!");
                  } else if (
                    stringToEncode.toLowerCase().startsWith("imgur.com") ==
            ...
            onStop() {
              ...
              if (document.getElementById("avatarDecorations"))
                document.getElementById("avatarDecorations").remove();
              if (document.getElementById("changeProfileEffectButton"))
                document.getElementById("changeProfileEffectButton").remove();
              if (document.getElementById("profileEffects"))
                document.getElementById("profileEffects").remove();
