	...
	const data = {
		...
		regexMatches: {
			...
			profileEffects: containsProfileEffects(revealedText)
				...
				: null,
			profileFrame: containsProfileFrame(revealedText)
				? extractProfileFrame(revealedText)
				: null,
			profileV2: containsBanner(revealedText),
		},
		badges: BadgesStore.check(user.id)
			? BadgesStore.returnRespondingBadges(user.id)
					.map((x) => String(x.id))
					.join(", ")
			: "none",
	};
