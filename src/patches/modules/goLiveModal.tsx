...
function ConfigModal({ props, onClose, forceQuality }) {
	...
	function onApply() {
		...
		const streamConnection = connections
			.filter?.(
				(x) => x?.streamUserId == UserStore.getCurrentUser().id && x?.context == "stream"
			)
			.find(Boolean);
		...
		const audioConnection = connections
			.filter?.(
				(x) =>
					x?.userId == UserStore.getCurrentUser().id &&
					x?.context == "default" &&
					!x?.streamUserId
			)
			.find(Boolean);
		...
		patcher.after(finale.modules[0], "default", (_, [args], ret) => {
			...
			if (SettingsStore.get("ResolutionSwapper")) {
				...
				footerContent.children = (
					<FooterColumn>
						<FooterRow>{originalChildren}</FooterRow>
					</FooterColumn>
				);
			}
