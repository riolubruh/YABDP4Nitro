...
function AdvancedNameplateTextInput({skuId, setSkuId, palette, setPalette}){
    ...
    function onKeyDown(e){
        if(e.keyCode == 13 || e.key == "Enter") return copyNameplate3y3({skuId: skuId ?? customSkuTextBox, palette: palette ?? customPaletteTextBox});
        else {
...
function NameplateCategory({skuId, query, setSkuId, setPalette}){
    ...
    >
        ...
        {filteredProducts.map(x => <Nameplate3y3 product={x} setSkuId={setSkuId} setPalette={setPalette} />)}
    </div> : null;
}
...
function Nameplates(){
    ...
    return <div>
        {advancedProfileCustomization ? <AdvancedNameplateTextInput
            palette={palette}
        ...
        {Collections.map(x=><NameplateCategory skuId={x} query={query} setSkuId={setSkuId} setPalette={setPalette} />)}
    </div>
}