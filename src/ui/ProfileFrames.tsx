		...
		>
			...
			>
				{filteredProducts.map((x) => (
					<ProfileFrame product={x} setSkuId={setSkuId} />
				))}
			</div>
		</div>
	) : null;
...
function ProfileFrames() {
	...
	return (
		<Suspense
			fallback={
				<div>
					This could be infinite loading situation, Please load the normal profile effects
					button
				</div>
			}
		>
			...
			{Collections.map((x) => (
				<ProfileFrameCategory skuId={x} query={query} setSkuId={setSkuId} />
			))}
		</Suspense>
	);
}
