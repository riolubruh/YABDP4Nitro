...
export default new class ShopCollectiblesStore extends BetterDiscord.Utils.Store {
    ...
    async fetch() {
        const [collections, quests] = await Promise.all([
            BetterDiscord.Net.fetch("https://raw.githubusercontent.com/aamiaa/discord-api-diff/refs/heads/main/collectibles.json").then(r => r.json() as Promise<ShopCollection[]>),
            BetterDiscord.Net.fetch("https://raw.githubusercontent.com/aamiaa/discord-api-diff/refs/heads/main/quests.json").then(r => r.json() as Promise<Quest[]>),
        ]);
    ...
    getAvatarDecorations(skuId: string) {
        return itemsByType<AvatarDecorationItem>(this.getCategory(skuId), ShopItemType.AvatarDecoration);
    }
