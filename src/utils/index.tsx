...
function getRevealedTextFromCustomStatus(
	userId: string,
	...
	try {
		...
		BetterDiscord.Logger.error(
			"Something went wrong getting custom status, oh god oh shit!",
			err
		);
		return undefined;
	}
...
export const EMOJI_URL_REGEX =
	/https:\/\/cdn\.discordapp\.com\/emojis\/\d+\.(png|webp|gif|avif|jpg|jpeg).*?(?=$| )/gi;
...
export function getBannerUrl(userId: string) {
	...
	return matched
		...
		: UserBackgroundStore.hasHash(userId)
			? UserBackgroundStore.format(userId)
			: null;
}
